...
// async function getExportedProxies(pathToProxies){
//     let list = await utils.getExportedEntities(pathToProxies).catch(function () {
//         return console.log("If not exported already then please export the proxies first.");
//     });
//     return list;
// }


async function start(pathToProxies) {
    utils.getExportedEntities(pathToProxies).then(async (list)=>{
        if(list){
            ...
            if(list[0] === toBeImported[0]){
                ...
                let taggerForExport = await performCorrection(list, toBeImported);
                if(taggerForExport.length <= 0){
                    return console.log("No proxies were tagged for export");
                }
                importProxies(taggerForExport);
            }
        }
        else{
    ...
    // let list = await getExportedProxies(pathToProxies);
    // if(list){
    //     importProxies(list);
    // }
    // else{
...
start(pathToProxies);
//module.exports.start = start(pathToProxies);
...
function checkForExistence(exportedProxies) {
    ...
    return new Promise(async function (resolve, reject) {
        ...
        if(alreadyOnTarget.length > 0){
            ...
            //prompt with exists
            let promptObject = utils.createPromptObject('exists', 'These proxies already exist on target org "' + config.to.org + '"');
