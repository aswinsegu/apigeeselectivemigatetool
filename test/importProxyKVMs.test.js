...
describe('Testing importProxyKVMs', function () {
    ...
    describe('Testing extractProxyListFromExports(str)', function () {

        this.timeout(5000);

        it('should return back the proxy list from provided path list', async function () {

            let pathToKVMs = options.dataDir.proxyKvmDir;

            let list = await utils.getExportedEntities(pathToKVMs);

            let envList = await importProxyKVMs.extractProxyListFromExports('' + list + '');

            expect(envList).to.be.an('array');
            expect(envList[0]).to.be.a('string');
        });

        it('should not return proxy list if proper paths are not given', async function () {
