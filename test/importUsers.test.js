...
describe('Testing importUsers', function () {
    ...
    describe('Testing getUsers()', function () {
        ...
        it('should return back the list of users from the target organization', async function () {

            let userList = JSON.parse(await importUsers.getUsers());

            expect(userList).to.be.an('array');
            expect(userList).to.have.property('length');
            expect(userList[0]).to.be.a('string');
        });
    });
